...
export async function scrapePlex(shared?: SharedScraperContext): Promise<number> {
  if (!shared) {
  ...
  try {
    logStep('SYSTEM', 'Starting plex script execution (shared session).');
    await page2.goto('https://matrix.centris.ca/Matrix/Recherche/Propri%C3%A9t%C3%A9%C3%A0revenus/G%C3%A9n%C3%A9rale');
    ...
    await page2.locator('input[name="Fm103_Ctrl3425_TB"]').click();
    await humanDelay(page2, 1000, 1500);
    await page2.locator('input[name="Fm103_Ctrl3425_TB"]').fill(searchDateValue);
  ...
  logStep('EXPORT', `Loaded ${existingReferenceNumbers.size} existing reference number(s) from Google Sheets for duplicate checks.`);
    ...
    const returnToResultsGrid = async () => {
      const listingLocator = page2.locator("td.d15880m6 a[data-mtx-track='Results - In-Display Full Link Click']");
    ...
    const processOpenedListing = async (listingId: string, pageIndex: number, listingIndex: number) => {
      ...
      const rawPrice = await page2.locator('.field.d15836m20').first().textContent();
    ...
    for (let i = 0; i < rowCount; i++) {
      const row = page2.locator('td.d15880m6').nth(i);
      const mainLink = row.locator("a[data-mtx-track='Results - In-Display Full Link Click']").first();
      if (!await mainLink.isVisible().catch(() => false)) continue;
    ...
    const getCurrentListingId = async (fallback: string): Promise<string> => {
      const rawCentris = await page2.locator('.field.formula.d15836m5').first().textContent().catch(() => null);
...
if (require.main === module) {
  Promise.race([
    (async () => {
      ...
      const logger = createExecutionLogger('plex');
      try {
