...
export async function scrapeResidential(shared?: SharedScraperContext): Promise<number> {
	if (!shared) {
	...
	try {
		logStep('SYSTEM', 'Starting unifamilial script execution (shared session).');
		// await page2.getByLabel('Main menu').getByRole('button', { name: 'Recherche' }).click();
		// await humanDelay(page2, 500, 1000);
		// await page2.getByRole('link', { name: 'Générale' }).first().click();
		// await humanDelay(page2, 1000, 1500);
		await page2.goto('https://matrix.centris.ca/Matrix/Recherche/Unifamiliale/G%c3%a9n%c3%a9rale');

		// Step 11: Fill in search criteria - enter search date field
		...
		await page2.locator('input[name="Fm43_Ctrl3416_TB"]').click();
		await humanDelay(page2, 1000, 1500);
		await page2.locator('input[name="Fm43_Ctrl3416_TB"]').fill(searchDateValue);
		// await page2.locator('input[name="Fm43_Ctrl3416_TB"]').fill('20260403+');
		...
		const processOpenedListing = async (listingId: string, pageIndex: number, listingIndex: number) => {
			...
			// Step 15: Extract data from the listing details page
			...
			const allFields = await page2.locator('.field.d156m18').allTextContents();
			const addressFields = allFields.filter(text => text && text.trim().length > 0 && !text.includes('$'));

			const {
				streetNumber: otherStreetNumber,
				streetName: otherStreetName,
				appartment: otherAppartment,
			} = parseStreetAddress(addressFields[0]);
			...
			for (let i = 0; i < docCount; i++) {
				...
				if (download) {
					...
					// metadata-only detection; no verbose console output
				} else {
					...
					if (openedPage) {
						try {
							...
							// popup fetch detection; no verbose console output
			...
			const onMatrixResponse = async (response: Response) => {
				try {
					...
					// Preferred strict source: Chrome PDF viewer UUID stream + PDF MIME, plus direct PrintPDF PDF MIME in headless.
					const isStrictCandidate = (isChromeViewerPdfUuid && isPdfMime) || isHeadlessPrintPdf;
					// Compatibility fallback: old behavior accepted backend PrintPDF responses when MIME/signature are valid.
					...
					if (isPdfSig && body.length > 500) {
						matrixCandidates.push({
							source: `network:${response.url() || 'unknown'}`,
							size: body.length,
							buffer: body,
							quality
						});
			...
			// Important: attach before click so we don't miss fast responses.
			...
			try {
				...
				for (let attempt = 1; attempt <= 3 && !bestCandidate; attempt++) {
					...
					if (attempt > 1) {
						await capturePage.reload({ waitUntil: 'domcontentloaded', timeout: 60000 }).catch(() => null);
					...
					for (let i = 0; i < 60; i++) {
						...
						if (bestCandidateSize === lastBestSize && bestCandidateSize > 20000) {
							stableTicks++;
						} else {
							stableTicks = 0;
						}
						...
						// Keep listening long enough for the backend to finish full chunk assembly (observed ~12s).
					...
					bestCandidate = (strictCandidates.length ? strictCandidates : legacyCandidates)
				...
				if (bestCandidate) {
					...
					const fallbackUrls = Array.from(new Set([
						(matrixPopup?.url() || '').trim(),
						...Array.from(matrixFetchUrls)
					])).filter(u => !!u && !u.startsWith('chrome-extension://'));
					...
					for (const fallbackUrl of fallbackUrls) {
						...
						for (let retry = 1; retry <= 3; retry++) {
							try {
								...
								const fallbackResp = await page2.context().request.get(fallbackUrl, {
									timeout: 60000,
			...
			// Back from print page to listing details page
			...
			// now info scrap info details
			const mailingAddressResults = (await page3.locator('.field.d12023m4').allTextContents()).map(v => (v || '').trim());

			// Requested indexing logic:
			// 1) If [1] is not empty -> fullName=[1], address=[3], cityZip=[5]
			// 2) Else -> fullName=[0], address=[2], cityZip=[4]
			...
			// Keep only the phone substring from first digit to last digit (drops labels like "Cellulaire:").
			const phoneNumber = (() => {
		...
		await page2.locator('#m_pnlDisplay').waitFor({ state: 'visible', timeout: 60000 }).catch(() => null);
		...
		for (let i = 0; i < rowCount; i++) {
			const row = page2.locator('td.d15872m6').nth(i);
			const mainLink = row.locator("a[data-mtx-track='Results - In-Display Full Link Click']").first();
			if (!await mainLink.isVisible().catch(() => false)) {
				continue;
			}

			const listingId = ((await mainLink.textContent()) || '').trim() || `row_${i + 1}`;
			const markerCount = await row.locator("a[data-original-title*='Il existe une inscription en vigueur pour cette propriété.']").count();
			if (markerCount > 0) {
		...
		if (!firstListingOpened) {
			...
			logStep('SYSTEM', `Script completed. Leads processed: ${leadsProcessed}`);
			return leadsProcessed;
		}

		const getCurrentListingId = async (fallback: string): Promise<string> => {
			const rawCentris = await page2.locator('.formula.field.d156m5').first().textContent().catch(() => null);
			return extractFirstWord(rawCentris || '') || fallback;
		};

		const hasActiveInscriptionMarkerOnDetail = async (): Promise<boolean> => {
			const activeMarker = page2.locator("a[data-original-title*='Il existe une inscription en vigueur pour cette propriété.']").first();
			if (await activeMarker.isVisible().catch(() => false)) return true;
		...
		const moveToNextListing = async (): Promise<boolean> => {
			const nextListingButton = page2.locator('#m_DisplayCore_dpy3').first();
			const hasNextButton = await nextListingButton.count();
			if (!hasNextButton) {
				logStep('NAVIGATION', 'Next listing button (#m_DisplayCore_dpy3) not found so its the last listing for this category. Stopping.');
				return false;
			}
...
if (require.main === module) {
	Promise.race([
		(async () => {
			...
			const logger = createExecutionLogger('unifamilial');
			try {
