...
export async function scrapeCopropriete(shared?: SharedScraperContext): Promise<number> {
	if (!shared) {
	...
	try {
		logStep('SYSTEM', 'Starting copropriete script execution (shared session).');
		await page2.goto('https://matrix.centris.ca/Matrix/Recherche/Copropri%C3%A9t%C3%A9AppartementR%C3%A9sidentiel/G%C3%A9n%C3%A9rale');

		// Step 11: Fill in search criteria
		...
		await page2.locator('input[name="Fm110_Ctrl3425_TB"]').click();
		await humanDelay(page2, 1000, 1500);
		await page2.locator('input[name="Fm110_Ctrl3425_TB"]').fill(searchDateValue);
		// await page2.locator('input[name="Fm110_Ctrl3425_TB"]').fill('20260405+');
		...
		const returnToResultsGrid = async () => {
			const listingLocator = page2.locator("td.d15877m6 a[data-mtx-track='Results - In-Display Full Link Click']");
			...
			await listingLocator.first().waitFor({ state: 'visible', timeout: 60000 }).catch(() => null);
		...
		const processOpenedListing = async (listingId: string, pageIndex: number, listingIndex: number) => {
			...
			const rawPrice = await page2.locator('.field.formula.d15833m20').first().textContent();
			...
			const rawCentris = await page2.locator('.formula.field.d15833m5').first().textContent();
			...
			const addressFields = await page2.locator('.field.d15833m20').allTextContents();
			const {
		...
		for (let i = 0; i < rowCount; i++) {
			const row = page2.locator('td.d15877m6').nth(i);
			const mainLink = row.locator("a[data-mtx-track='Results - In-Display Full Link Click']").first();
			if (!await mainLink.isVisible().catch(() => false)) continue;
		...
		const getCurrentListingId = async (fallback: string): Promise<string> => {
			const rawCentris = await page2.locator('.formula.field.d15833m5').first().textContent().catch(() => null);
		...
		const moveToNextListing = async (): Promise<boolean> => {
			...
			if (!hasNextButton) {
				logStep('NAVIGATION', 'Next listing button (#m_DisplayCore_dpy3) not found. Stopping.');
				return false;
			}
...
if (require.main === module) {
	Promise.race([
		(async () => {
			...
			const logger = createExecutionLogger('copropriete');
			try {
